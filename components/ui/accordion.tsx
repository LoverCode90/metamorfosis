...
function AccordionItem({ className, ...props }: AccordionPrimitive.Item.Props) {
  return (
    <AccordionPrimitive.Item
      ...
      className={cn(
        "not-last:border-border-subtle not-last:border-b",
        className,
      )}
      {...props}
    />
  ...
  return (
    <AccordionPrimitive.Header className="flex">
      <AccordionPrimitive.Trigger
        ...
        className={cn(
          "group/accordion-trigger text-foreground relative flex flex-1 items-start justify-between rounded-md border border-transparent py-3 text-left text-sm font-medium transition-all outline-none",
...
function AccordionContent({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "text-muted-foreground h-(--accordion-panel-height) pt-0 pb-3",
