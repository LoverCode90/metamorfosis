import { Check, MapPin, Package, Store, XCircle } from "lucide-react"

import { pickupOrderStatusToStageIndex } from "@/lib/orders/types"
...
export function PickupTrackingStages({ order }: { order: DbOrder }) {
  const currentStage = pickupOrderStatusToStageIndex(order.status)
  const isCancelled =
      ...
      {isCancelled ? (
        ...
        <ol className="flex flex-col">
          {PICKUP_STAGES.map((stage, i) => {
            ...
            const isLast = i === PICKUP_STAGES.length - 1
