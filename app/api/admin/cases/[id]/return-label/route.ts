...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!shippoTransactionId) {
      return NextResponse.json({ error: "Original order has no Shippo transaction ID" }, { status: 400 })
    }
    ...
    try {
      ...
      return NextResponse.json({ error: "Shippo return label generation failed", details: err.message }, { status: 500 })
    }

    const labelUrl = shippoRes.label_url
    if (!labelUrl) {
      return NextResponse.json({ error: "Shippo did not return a label_url" }, { status: 500 })
    }
